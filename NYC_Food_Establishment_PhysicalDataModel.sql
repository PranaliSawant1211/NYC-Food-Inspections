 ...
 *
 * Date Created : Monday, February 13, 2023 22:35:49
...
/* 
 * VIEW: Address_Restuarant 
 */

CREATE VIEW Address_Restuarant
(RestaurantName, Country, State, City, StreetAddress, BuildingNumber)  AS
SELECT Dim.RestaurantName, Di.Country, Di.State, Di.City, Di.StreetAddress, Di.BuildingNumber
FROM nyc_ins.DimRestaurant Dim, nyc_ins.DimAddress Di
WHERE Di.RestaurantSK = Dim.RestaurantSK
go

IF OBJECT_ID('Address_Restuarant') IS NOT NULL
    PRINT '<<< CREATED VIEW Address_Restuarant >>>'
ELSE
    PRINT '<<< FAILED CREATING VIEW Address_Restuarant >>>'
go
